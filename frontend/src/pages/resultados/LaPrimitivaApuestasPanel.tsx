...
export function LaPrimitivaApuestasPanel() {
  const [windowKey, setWindowKey] = useState<ApuestasWindow>('3m');
  const { points: allPoints, loading, error } = useApuestasSeries('la-primitiva', windowKey);
  const [weekdayFilter, setWeekdayFilter] = useState<'all' | 'lunes' | 'jueves' | 'sabado'>('all');
  ...
  const points = (() => {
    ...
    return allPoints.filter((p) => {
      ...
      const day = jsDate.getDay(); // 0=Sun,1=Mon,4=Thu,6=Sat
      if (weekdayFilter === 'lunes') return day === 1;
      if (weekdayFilter === 'jueves') return day === 4;
      if (weekdayFilter === 'sabado') return day === 6;
      return true;
    });
  })();
  ...
  return (
    <section className="card resultados-apuestas-card">
      <div className="resultados-apuestas-header">
        <h2 style={{ margin: 0, fontSize: '1rem' }}>Gráficos La Primitiva</h2>
        <div className="resultados-apuestas-controls">
          ...
          <div className="resultados-apuestas-weekday-toggle" aria-label="Filtrar por día de la semana">
            ...
            <button
              type="button"
              className={`resultados-apuestas-window-btn ${
                weekdayFilter === 'lunes' ? 'resultados-apuestas-window-btn--active' : ''
              }`}
              onClick={() => setWeekdayFilter('lunes')}
            >
              Lunes
            </button>
            <button
              type="button"
              className={`resultados-apuestas-window-btn ${
                weekdayFilter === 'jueves' ? 'resultados-apuestas-window-btn--active' : ''
              }`}
              onClick={() => setWeekdayFilter('jueves')}
            >
              Jueves
            </button>
            <button
              type="button"
              className={`resultados-apuestas-window-btn ${
                weekdayFilter === 'sabado' ? 'resultados-apuestas-window-btn--active' : ''
              }`}
              onClick={() => setWeekdayFilter('sabado')}
            >
              Sábado
            </button>
          </div>
      ...
      {loading && !hasData && <p style={{ marginTop: 'var(--space-sm)' }}>Cargando histórico…</p>}
      {!loading && !error && !hasData && (
        <p style={{ marginTop: 'var(--space-sm)' }}>No hay datos para este rango.</p>
      )}
      ...
      {hasData && (
        <>
          ...
          <div style={{ width: '100%', height: 320, marginTop: 'var(--space-sm)' }}>
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={points} margin={{ top: 10, right: 40, left: 0, bottom: 20 }}>
                <CartesianGrid strokeDasharray="3 3" />
                <XAxis
                  dataKey="date"
                  tick={{ fontSize: 10 }}

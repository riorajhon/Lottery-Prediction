...
function formatDateWithWeekday(date: string, weekday?: string) {
  if (!date) return '—';
  try {
...
function EuromillonesFeaturesTableRow({
  ...
  const prevLabel = row.prev_draw_date
    ? formatDateWithWeekday(row.prev_draw_date, row.prev_weekday || undefined)
    : '—';
  ...
  return (
    <tr>
      ...
      <td>
        {row.prev_main_numbers && row.prev_main_numbers.length > 0 ? (
          <>
            <div>{prevLabel}</div>
            <NumbersPillList values={row.prev_main_numbers} />
            {row.prev_star_numbers && row.prev_star_numbers.length > 0 && (
              <>
                {' '}
                (
                <NumbersPillList values={row.prev_star_numbers} />
                )
              </>
            )}
          </>
...
export function EuromillonesFeaturesPanel() {
  ...
  const loadGapsForDate = async (endDateStr: string) => {
    ...
    try {
      ...
      setGapError(e instanceof Error ? e.message : 'Failed to load gap history');
